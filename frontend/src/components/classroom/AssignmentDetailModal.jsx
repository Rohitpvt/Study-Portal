...
const AssignmentDetailModal = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-5xl glass dark:bg-[#0a0a0a] rounded-[4rem] border border-white/60 dark:border-white/5 overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-500 flex flex-col h-[90vh] max-h-[900px]">
        {/* Header */}
        <div className="px-12 py-10 flex items-start justify-between">
           <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-3">
                 <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${dueStatus.bg} ${dueStatus.color}`}>
                   {dueStatus.label}
                 </span>
                 {assignment.status === 'draft' && (
                   <span className="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-slate-100 dark:bg-white/5 text-slate-500">
                     Draft Post
                   </span>
                 )}
                 <div className="flex items-center gap-1.5 px-4 py-1.5 bg-indigo-500/10 text-indigo-500 rounded-full text-[10px] font-black uppercase tracking-widest">
                    <Target className="w-3.5 h-3.5" />
                    {assignment.points} Points
                 </div>
              </div>
              <h2 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter max-w-2xl leading-tight">
                {assignment.title}
              </h2>
              <div className="flex items-center gap-4 text-sm font-bold text-slate-500">
                ...
                {assignment.due_at && (
                   <div className="flex items-center gap-2">
                      ...
                      Due {new Date(assignment.due_at).toLocaleString([], { dateStyle: 'long', timeStyle: 'short' })}
                   </div>
                )}
              </div>
           </div>
           ...
           {/* Tabs */}
           <div className="flex items-center gap-2 p-1.5 glass dark:bg-white/5 rounded-2xl border border-white/5 ml-12">
              <button
                onClick={() => setActiveSubTab('instructions')}
                className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'instructions' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
              >
                Instructions
              </button>
              <button
                onClick={() => setActiveSubTab('work')}
                className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'work' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
              >
                {canManage ? 'Submissions' : 'Your Work'}
              </button>
              <button
                onClick={() => setActiveSubTab('comments')}
                className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'comments' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
              >
                Comments
              </button>
           </div>
           ...
           <button 
            onClick={onClose}
            className="p-4 hover:bg-slate-100 dark:hover:bg-white/5 rounded-3xl transition-all text-slate-400"
           >
             <X className="w-8 h-8" />
           </button>
        </div>
